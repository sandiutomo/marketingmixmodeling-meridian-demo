...
export default function ModelDiagnosticsPanel({ modelResults }: ModelDiagnosticsPanelProps) {
  ...
  const maxRhat = realRhatMax ?? (diagnostics.length > 0 ? Math.max(...diagnostics.map(d => d.rhat)) : null)
  const minEss  = realEssMin  ?? (diagnostics.length > 0 ? Math.min(...diagnostics.map(d => d.ess))  : null)
  ...
  const healthChecks = modelResults && health ? [
    {
      ...
      pass: (maxRhat ?? 999) < 1.2,
      review: (maxRhat ?? 999) < 1.5,
      guidance: 'R-hat is above 1.2 but below 1.5 — the 4 chains didn\'t fully agree. Try increasing n_adapt (warmup steps) to 1500 or n_burnin to 1000, or check for highly correlated channels in your spend data.',
    },
    {
  ...
  return (
    <div className="card card-body space-y-5">
      ...
      {/* R-hat chart — mirrors Meridian docs model-diagnostics view */}
      {rhatChartData.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-slate-600 mb-2">
            R-hat per parameter
            <span className="ml-2 font-normal text-slate-400">{isRealMeridian ? 'real posterior' : 'illustrative'} · green &lt; 1.1 · amber 1.1–1.2 · red ≥ 1.2</span>
          </p>
      ...
      {/* Actual vs Predicted — model fit quality chart */}
      {weeklyChartData.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-slate-600 mb-1">
            Actual vs Predicted revenue (last 26 weeks)
            <span className="ml-2 font-normal text-slate-400">{isRealMeridian ? 'posterior predictive' : 'illustrative'}</span>
          </p>
      ...
      {/* Per-parameter MCMC details (collapsed by default) — only when model results are available */}
      ...
      <details>
        <summary className="text-xs font-medium text-slate-500 cursor-pointer hover:text-slate-700 select-none flex items-center gap-1.5">
          ...
          <span className="text-slate-300">({diagnostics.length} parameters · {isRealMeridian ? 'real posterior' : 'illustrative'})</span>
        </summary>
