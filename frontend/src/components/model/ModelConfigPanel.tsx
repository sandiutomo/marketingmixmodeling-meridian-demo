          ...
          <details className="group">
            ...
            <div className="mt-3 p-4 bg-surface-50 rounded-xl space-y-5">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-[8rem_1fr] gap-x-4 gap-y-4 items-start">
                  <input
                    type="number" min={1} max={13} value={config.maxLag}
                    onChange={e => { setApplied(false); setConfig(p => ({ ...p, maxLag: parseInt(e.target.value) || 8 })) }}
                    className="w-full px-3 py-1.5 text-sm border border-surface-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-200"
                  />
                  <div>
                  ...
                  <select
                    ...
                    className="w-full px-2 py-1.5 text-sm border border-surface-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-200"
                  >
                  ...
                  <div className="flex items-center pt-0.5">
                    <input
                      type="checkbox"
                      id="hillBeforeAdstock"
                      checked={config.hillBeforeAdstock ?? false}
                      onChange={e => { setApplied(false); setConfig(p => ({ ...p, hillBeforeAdstock: e.target.checked })) }}
                      className="rounded border-surface-300 w-4 h-4 cursor-pointer"
                    />
                  </div>
                  <div>
                    <label htmlFor="hillBeforeAdstock" className="text-xs font-semibold text-slate-700 cursor-pointer">Hill before adstock</label>
                    <p className="text-xs text-slate-400 mt-0.5 leading-relaxed">Apply the Hill saturation curve <em>before</em> the adstock (carryover) transformation instead of after. Most use cases keep this off (adstock first). Turn on if you believe diminishing returns occur before the carry-over effect in your category — e.g. some TV markets. <code className="font-mono">hill_before_adstock</code></p>
                  </div>
                  <div className="flex items-center pt-0.5">
                    <input
                      type="checkbox"
                      id="uniqueSigmaPerGeo"
                  ...
                  <div>
                    <label htmlFor="uniqueSigmaPerGeo" className="text-xs font-semibold text-slate-700 cursor-pointer">Unique σ per geo</label>
                    <p className="text-xs text-slate-400 mt-0.5 leading-relaxed">Fit a separate revenue noise parameter for each geographic region. Useful when regions differ significantly in scale or variance. Adds model parameters — can slow convergence and requires more data per geo. Leave off for national models or similarly-sized regions. <code className="font-mono">unique_sigma_for_each_geo</code></p>
                  </div>
                  <input
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-[8rem_1fr] gap-x-4 gap-y-4 items-start">
                  <input type="number" value={config.nAdapt}
                    ...
                    className="w-full px-3 py-1.5 text-sm border border-surface-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-200" />
                  <div>
                    <p className="text-xs font-semibold text-slate-700">Model Calibration Runs</p>
                    <p className="text-xs text-slate-400 mt-0.5 leading-relaxed">These runs help the model adjust itself to your data so it can make accurate estimates. Default 1000.</p>
                  </div>
                  <input type="number" value={config.nBurnin}
                    onChange={e => { setApplied(false); setConfig(p => ({...p, nBurnin: parseInt(e.target.value)})) }}
                    className="w-full px-3 py-1.5 text-sm border border-surface-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-200" />
                  <div>
                    <p className="text-xs font-semibold text-slate-700">Stabilization Runs</p>
                    <p className="text-xs text-slate-400 mt-0.5 leading-relaxed">Initial runs where the model is still settling in — these are not used in the final results. Default 500.</p>
                  </div>
                  <input type="number" value={config.nKeep}
                    onChange={e => { setApplied(false); setConfig(p => ({...p, nKeep: parseInt(e.target.value)})) }}
                    className="w-full px-3 py-1.5 text-sm border border-surface-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-200" />
                  <div>
                    <p className="text-xs font-semibold text-slate-700">Confidence Runs</p>
                    <p className="text-xs text-slate-400 mt-0.5 leading-relaxed">These runs are used to calculate your final results. More runs mean more reliable estimates.</p>
                  </div>
                </div>
              </div>
