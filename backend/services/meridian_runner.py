    ...
    def fit(self, data: dict, config: dict, progress_callback: ProgressCb = None) -> dict:
        ...
        config : dict from ModelRunnerService._config
                 Keys: nChains, nAdapt, nBurnin, nKeep, maxLag, adstockDecay,
                       mediaPriorType.
        """
        MeridianRunner._last_results = None  # clear before MCMC so a failed re-run doesn't serve stale data
        t0 = time.time()
        ...
        # NOTE: with_media(m, ms, channels) — we pass spend for both m (media) and ms
        # (media_spend). This means media_units == spend_units throughout: Hill ec,
        # adstock media_units, and scenario spend values are all in the same currency.
        if media_channels:
            if has_geo:
        ...
        # ── Per-geo ROI ───────────────────────────────────────────────────────
        logger.info("[MeridianRunner] Extracting per-geo ROI…")
        geo_roi_list: list = []
        try:
            from services.results_generator import CHANNEL_DISPLAY as _DISP
        except Exception:
            _DISP = {}
        try:
            _geos    = data.get('geos', ['national'])
            _spend_g = data['spend_data']   # (n_geos, n_times, n_ch) or (n_times, n_ch)
            _kpi_g   = data['kpi_data']     # (n_geos, n_times) or (n_times,)
            _ch_keys = data['channels']

            if len(_geos) > 1 and _spend_g.ndim == 3:
                for g_idx, geo in enumerate(_geos):
                    try:
                        try:
                            geo_ds = analyzer.summary_metrics(
                                confidence_level=0.90, selected_geos=[geo],
                            )
                        except TypeError:
                            # selected_geos not supported in this Meridian version — stop trying
                            logger.info("[MeridianRunner] selected_geos unsupported; skipping per-geo ROI")
                            break
                        geo_chs = [str(c) for c in geo_ds.channel.values
                                   if str(c) != 'All Paid Channels']

                        ch_rows: list = []
                        media_rev = 0.0
                        for ch in geo_chs:
                            try:
                                roi_v = float(geo_ds['roi'].sel(
                                    distribution='posterior', channel=ch, metric='mean'))
                                rev_v = float(geo_ds['incremental_outcome'].sel(
                                    distribution='posterior', channel=ch, metric='mean'))
                                ch_idx = _ch_keys.index(ch) if ch in _ch_keys else -1
                                sp_v   = float(_spend_g[g_idx, :, ch_idx].sum()) if ch_idx >= 0 else 0.0
                                ch_rows.append({
                                    'channel': _DISP.get(ch, ch),
                                    'roi':     round(roi_v, 4),
                                    'spend':   round(sp_v),
                                    'revenue': round(rev_v),
                                })
                        ...
                        geo_spend = float(_spend_g[g_idx].sum())
                        geo_rev   = float(_kpi_g[g_idx].sum()) if _kpi_g.ndim == 2 else float(_kpi_g.sum())
                        p_roi     = media_rev / geo_spend if geo_spend > 0 else 0.0

                        geo_roi_list.append({
                            'geo':          geo,
                            'totalRevenue': round(geo_rev),
                            'totalSpend':   round(geo_spend),
                            'portfolioRoi': round(p_roi, 4),
                            'mediaRevenue': round(media_rev),
                            'baseRevenue':  round(max(0.0, geo_rev - media_rev)),
                            'channels':     ch_rows,
                            'isReal':       True,
                        })
                    except Exception as e:
                        logger.warning("[MeridianRunner] Geo ROI for %s failed: %s", geo, e)

                if geo_roi_list:
                    geo_roi_list.sort(key=lambda r: r['portfolioRoi'], reverse=True)
                    logger.info("[MeridianRunner] Per-geo ROI: %d geos extracted", len(geo_roi_list))
        except Exception as e:
            logger.warning("[MeridianRunner] Per-geo ROI extraction failed: %s", e)
            geo_roi_list = []
